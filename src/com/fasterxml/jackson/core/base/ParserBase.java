...
public abstract class ParserBase extends ParserMinimalBase {
	...
	protected ParserBase(IOContext ctxt, int features) {
		...
		DupDetector dups = Feature.STRICT_DUPLICATE_DETECTION.enabledIn(features) ? DupDetector.rootDetector(this)
				: null;
	...
	public JsonParser enable(Feature f) {
		_features |= f.getMask();
		if (f == Feature.STRICT_DUPLICATE_DETECTION) { // enabling dup
														// detection?
			if (_parsingContext.getDupDetector() == null) { // but only if
															// disabled
															// currently
	...
	public JsonLocation getTokenLocation() {
		return new JsonLocation(_ioContext.getSourceReference(), -1L, getTokenCharacterOffset(), // bytes,
																									// chars
	...
	public JsonLocation getCurrentLocation() {
		...
		return new JsonLocation(_ioContext.getSourceReference(), -1L, _currInputProcessed + _inputPtr, // bytes,
																										// chars
				_currInputRow, col);
	}
	...
	public float getFloatValue() throws IOException {
		 ...
		 */
		/*
		 * if (value < -Float.MAX_VALUE || value > MAX_FLOAT_D) { _reportError(
		 * "Numeric value ("+getText()+") out of range of Java float"); }
		 */
		return (float) value;
	}
	...
	protected void reportOverflowInt() throws IOException {
		_reportError(String.format("Numeric value (%s) out of range of int (%d - %s)", getText(), Integer.MIN_VALUE,
				Integer.MAX_VALUE));
	}

	protected void reportOverflowLong() throws IOException {
		_reportError(String.format("Numeric value (%s) out of range of long (%d - %s)", getText(), Long.MIN_VALUE,
				Long.MAX_VALUE));
	}
	...
	/*
	 * /********************************************************** /* Stuff that
	 * was abstract and required before 2.8, but that /* is not mandatory in 2.8
	 * or above. /**********************************************************
	 */
